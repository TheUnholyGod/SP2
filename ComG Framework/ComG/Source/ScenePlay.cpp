...
ScenePlay::ScenePlay() : buildingID(101)
{
	...
	windowX = windowY = 0;
	cursorPoint1.x = cursorPoint1.y = 0;
...
void ScenePlay::Update(double dt)
{
	DebugMode(dt);
...
void ScenePlay::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void ScenePlay::DebugMode(double dt)
{
...
void ScenePlay::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void ScenePlay::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
...
void ScenePlay::SpawnEnemy(double dt)
{
	if (BaseEnemy.size() < 5)
...
void ScenePlay::SpawnBuilding(double dt)
{
...
void ScenePlay::RenderMeshOnScreen(Mesh* mesh, int x, int y, int sizex, int sizey)
{
	glDisable(GL_DEPTH_TEST);
	Mtx44 ortho;
...
void ScenePlay::LightUpdate(double dt)
{
	if (!reset)
	{
		suntimer = 1;
...
void ScenePlay::LightReset(double dt)
{
	suntimer = 20;
	reset = true;
}

void ScenePlay::buildBuildingUpdate(double dt)
{
