...
bool LoadOBJ(const char *file_path, std::vector<Position> & out_vertices, std::vector<TexCoord> & out_uvs, std::vector<Vector3> & out_normals)
{
	...
	while (!fileStream.eof()) {
		...
		{ //process texcoord
			TexCoord texvertex;
			sscanf_s((buf + 3), "%f%f", &texvertex.u, &texvertex.v);
			temp_uvs.push_back(texvertex);

		}
		else if (strncmp("vn ", buf, 3) == 0)
		{ //process normal
			Vector3 nvertex;
			sscanf_s((buf + 2), "%f%f%f", &nvertex.x, &nvertex.y, &nvertex.z);
			temp_normals.push_back(nvertex);
