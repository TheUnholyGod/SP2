...
void SceneUI::LoadSkybox()
{
	meshList[GEO_FRONT] = MeshBuilder::GenerateQuad("front", Color(1, 1, 1), 1.f);
	meshList[GEO_FRONT]->textureID = LoadTGA("Image//left.tga");

	meshList[GEO_LEFT] = MeshBuilder::GenerateQuad("left", Color(1, 1, 1), 1.f);
	meshList[GEO_LEFT]->textureID = LoadTGA("Image//back.tga");

	meshList[GEO_RIGHT] = MeshBuilder::GenerateQuad("right", Color(1, 1, 1), 1.f);
	meshList[GEO_RIGHT]->textureID = LoadTGA("Image//front.tga");

	meshList[GEO_BACK] = MeshBuilder::GenerateQuad("back", Color(1, 1, 1), 1.f);
	meshList[GEO_BACK]->textureID = LoadTGA("Image//right.tga");
...
void SceneUI::RenderSkybox()
{
	...
	RenderMesh(meshList[GEO_LEFT], !sunup);
	modelStack.PopMatrix();
	modelStack.PushMatrix();
	modelStack.Rotate(-90, 0, 1, 0);
	modelStack.Translate(0, -0.5, 0);
	modelStack.Scale(1, 1, -1);
	RenderMesh(meshList[GEO_BOTTOM], !sunup);
	modelStack.PopMatrix();
	modelStack.PushMatrix();
	modelStack.Rotate(90, 0, 0, 1);
	modelStack.Rotate(90, 1, 0, 0);
	modelStack.Translate(0, 0.5, 0);
	RenderMesh(meshList[GEO_RIGHT], !sunup);
	modelStack.PopMatrix();
	modelStack.PushMatrix();
	modelStack.Translate(0.5, 0, 0);
	modelStack.Rotate(90, 0, 0, 1);
	modelStack.Rotate(180, 1, 0, 0);
	RenderMesh(meshList[GEO_BACK], !sunup);
	modelStack.PopMatrix();
	modelStack.PushMatrix();
	modelStack.Rotate(90, 0, 1, 0);
	modelStack.Translate(0, 0.5, 0);
	RenderMesh(meshList[GEO_TOP], !sunup);
	modelStack.PopMatrix();
	modelStack.Translate(-0.5, 0, 0);
	modelStack.Rotate(90, 0, 0, 1);
	RenderMesh(meshList[GEO_FRONT], !sunup);
	modelStack.PopMatrix();
	//modelStack.PushMatrix();
	//modelStack.Translate(5, 0, 0);
	//modelStack.Rotate(sunrotate, 0, 0, 1);
	//RenderMesh(meshList[GEO_SUN], true);
	//modelStack.PopMatrix();
	modelStack.PopMatrix();
}

void SceneUI::DebugMode(double dt)
{
...
void SceneUI::UpdateCharacter(double dt)
{
	if (!fps)
	{
...
void SceneUI::cameraupdates(double dt)
{
...
void SceneUI::LightUpdate(double dt)
{
	if (!reset)
	{
		suntimer = 1;
...
bool SceneUI::ifBounds(Vector3 forwhat)
{
	bool local = false;
	for (auto &i : walls)
	{
...
bool SceneUI::sleep(Vector3 forwhat)
{
...
void SceneUI::LightReset(double dt)
{
	suntimer = 20;
	reset = true;
}

void  SceneUI::growVege(double dt)
{
	for (auto &i : allvegetables)
	{
		if (!i->picked)
...
void SceneUI::updateGoat(double dt, Vector3 forwhat)
{
	for (auto &i : allgoats)
	{
		bool blocked = false;
...
void SceneUI::plantVege(int vegeno)
{
...
void SceneUI::vegepicked(Vector3 forwhat)
{
	for (auto &i : allvegetables)
	{
		if (i->growpotato > 1)
