...
void SceneWildLife::RenderMeshOnScreen(Mesh* mesh, int x, int y, int sizex, int sizey)
{
	glDisable(GL_DEPTH_TEST);
	Mtx44 ortho;
...
bool SceneWildLife::allbuildingcollision(GameObject* test)
{
...
void SceneWildLife::LightUpdate(double dt)
{
	if (!reset)
	{
		suntimer = 1;
...
void SceneWildLife::LightReset(double dt)
{
	suntimer = 20;
	reset = true;
}